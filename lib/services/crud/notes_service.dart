import 'package:notedown/services/crud/crud_exceptions.dart';
...
class NotesService {
  ...
  Future<DatabaseUser> creaeUser({required String email}) async {
    final db = _getDatabaseOrThrow();
    final results = await db.query(userTable,
        limit: 1, where: 'email=?', whereArgs: [email.toLowerCase()]);
...
class DatabaseNote {
  ...
  @override
  String toString() =>
      'Note, ID=$id, userId=$userId, isSynedWithCloud=$isSyncedWithCloud , text=$text';
